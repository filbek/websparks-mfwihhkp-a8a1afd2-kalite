...
export const NotificationCard: React.FC<NotificationCardProps> = ({
    ...
    const getIconBgClass = () => {
        if (notification.is_read) {
            return 'bg-secondary-200 text-secondary-600';
        }
        switch (color) {
            case 'primary':
    ...
    return (
        <div
            ...
            className={`flex items-start space-x-3 p-4 rounded-lg cursor-pointer transition-all ${notification.is_read
                    ...
                    : 'bg-primary-50 hover:bg-primary-100 border-l-4 border-primary-600'
                }`}
        >
            <div
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                    <div className="flex-1">
                        <h4 className={`text-sm font-medium mb-1 ${notification.is_read ? 'text-secondary-700' : 'text-secondary-900'
                            }`}>
                            {notification.title}
                        </h4>
                        <p className={`text-sm ${notification.is_read ? 'text-secondary-500' : 'text-secondary-700'
                            }`}>
