          ...
          <div>
            ...
            {trend && (
              <div className="flex items-center mt-2">
                <i className={`bi ${trend.isPositive ? 'bi-arrow-up' : 'bi-arrow-down'} text-sm mr-1 ${trend.isPositive ? 'text-success-600 dark:text-success-400' : 'text-danger-600 dark:text-danger-400'
                  }`}></i>
                <span className={`text-sm font-medium ${trend.isPositive ? 'text-success-600 dark:text-success-400' : 'text-danger-600 dark:text-danger-400'
                  }`}>
                  {Math.abs(trend.value)}%
                </span>
                <span className="text-sm text-secondary-600 dark:text-secondary-400 ml-1">geçen aya göre</span>
              </div>
            )}
          </div>
