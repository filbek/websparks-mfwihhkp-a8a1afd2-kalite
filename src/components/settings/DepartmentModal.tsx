...
import { DofLocation } from '../../hooks/useDofLocations';

interface DepartmentModalProps {
  department?: DofLocation;
  onSubmit: (data: Omit<DofLocation, 'id' | 'created_at' | 'updated_at'>) => Promise<void>;
  onCancel: () => void;
  loading?: boolean;
}
...
export const DepartmentModal: React.FC<DepartmentModalProps> = ({
  ...
  const validate = () => {
    ...
    if (!formData.label.trim()) {
      newErrors.label = 'Bölüm adı gereklidir';
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-xs text-secondary-500">
          Bu isim DÖF formlarında görünecektir
        </p>
      </div>
      ...
      <div>
        ...
        <p className="mt-1 text-xs text-secondary-500">
          {department
            ...
            : 'Otomatik olarak bölüm adından oluşturulur'}
        </p>
      </div>
      ...
      <div>
        ...
        />
        <p className="mt-1 text-xs text-secondary-500">
          Bölümlerin görüntülenme sırasını belirler (küçük sayılar önce gelir)
        </p>
      </div>
